...
movieNames = spark.read.option("sep", "|").option("charset", "ISO-8859-1")\
                .schema(movieNamesSchema).csv("datasets\\ml-100k\\u.item")

movies = spark.read.option("sep", "\t").schema(moviesSchema).csv("datasets\\ml-100k\\u.data")

ratings = movies.select("userID", "movieID", "rating")

# Prepare moviePairs dataframe
...
if (len(sys.argv) > 1):
    # Set threshold values to filter result table
    scoreThreshold = 0.97
    ...
    # Filter result table with thresholds and given movie
    ...
    for result in results:
        ...
        # Choose the other movieID of pair
